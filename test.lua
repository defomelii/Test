...
if IsGame then
    ...
    local function shouldDisableCollider(object)
        ...
        if object:IsA("BasePart") then
            if object.Name == "NoClip" or object.Name == "CylinderCollider" or object.Name == "NoClip_Collider" then
                return true
            end
        end
        -- Models
        if object:IsA("Model") then
            if object.Name == "Wall" or object.Name == "ShortWall" or object.Name == "WindowWall" or object.Name == "RopeWall" or object.Name == "Counter" or object.Name == "New_Crate" then
                return true
            end
            -- Also include any model that contains a descendant named "NoClip_Collider"
            for _, descendant in ipairs(object:GetDescendants()) do
                if descendant:IsA("BasePart") and descendant.Name == "NoClip_Collider" then
                    return true
                end
            end
        end
    ...
    local function findAndDisableColliders()
        -- Scan workspace for all models and parts that match
    ...
    -- ================== PERMANENT AUTO SKILLCHECK WITH FAKE FEEDBACK ==================
    local function showGreatJobFeedback()
        -- Simple notification
        Fluent:Notify({ Title = "Skillcheck", Content = "Great Job! (Auto-completed)", Duration = 2 })
        -- Play success sound from a known asset
        local sound = Instance.new("Sound")
        sound.SoundId = "rbxasset://sounds/electronicpingshort.wav" -- generic ping
        sound.Volume = 0.6
    ...
    local function hookSkillCheckRemote()
        ...
        remote.OnClientInvoke = function(...)
            ...
            if type(args[2]) == "table" and args[2].type then
                result = true   -- treadmill / circle success
            else
                result = "supercomplete"  -- horizontal perfect
            end
            showGreatJobFeedback()
            return result
        end
        ...
        for _, gui in ipairs(PlayerGui:GetChildren()) do
            if gui:GetAttribute("BarnabyArcadeSession") then
                ...
                if targetGapY then
                    ...
                    -- No gap info, just try to keep fish around middle
                    if fishY < -2 then
                        shouldJump = true
                    end
                end
    ...
    -- Auto Skillcheck button (permanent once enabled)
    AutoSkillcheckSection:AddButton({ 
        Title = "Enable Auto Skillchecks", 
        Description = "Permanently hooks the remote – cannot be undone until rejoin.", 
        Callback = function()
            if SkillCheckHooked then
                Fluent:Notify({ Title = "Already Enabled", Content = "Skillchecks are already auto-completed.", Duration = 2 })
                return
            end
            local success = hookSkillCheckRemote()
            if success then
                Fluent:Notify({ Title = "Success", Content = "Auto skillchecks activated! Perfect every time.", Duration = 3 })
    ...
    AutoSkillcheckSection:AddToggle("AutoBarnabyToggle", { 
        ...
        Description = "Smartly plays the Barnaby minigame (dodge gaps, collect coins)", 
        Default = false, 
        Callback = function(value)
            AutoBarnabyEnabled = value
            if value then
                Fluent:Notify({ Title = "Auto Barnaby", Content = "Barnaby auto-play enabled.", Duration = 2 })
            end
        end 
    })
end
